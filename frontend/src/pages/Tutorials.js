...
const TUTORIALS = [
  ...
  {
    id: "billing-entity-approval-matrix",
    title: "Billing Entities & Multi-Level Approval Matrix",
    description: "Configure per-Billing-Entity approval ladders and follow how projects move through Level 1 → Level 2 → … approvers sequentially.",
    duration: "5 min",
    category: "Approvals",
    updated: "Jul 2026 · Iter 78–79",
    icon: Shield,
    color: "bg-indigo-600",
    steps: [
      { target: "Open Master Data → Billing Entities.", action: "Add or verify each Yash legal entity (name, code, country). Every project must be tagged to one Billing Entity — it drives filtering, exports, and approval routing." },
      { target: "Go to Admin → Approval Matrix (admin-only).", action: "The link now lives in the Admin section of the sidebar. The top-right badge shows how many Approver-role users are available." },
      { target: "Pick a Billing Entity to configure.", action: "Select the entity from the dropdown. If a matrix already exists you'll see it; otherwise start fresh with Level 1." },
      { target: "Add up to 5 levels with approvers.", action: "For each level: give it a label like 'Delivery Head' or 'Finance Manager', then add one or more approvers from the users list. Multiple users in the same level act as an OR gate — the first to act advances the flow." },
      { target: "Submit a project for review.", action: "Open any project tagged to that billing entity, click Submit for Review. If a matrix exists the approver dropdown is hidden — a preview of every level + approver appears instead. Only Level 1 approvers get emailed." },
      { target: "Watch the Approval Progress tracker.", action: "A new collapsible card at the top of the Estimator (above Project Information) shows every level with live status: Approved (with approver, timestamp, comment), Awaiting (current level), or Pending." },
      { target: "Approve straight from the bell.", action: "Approvers can click the notification bell → Approve / Reject / Open. Approve opens a comment box in-place — no need to open the project." },
      { target: "Follow up on stuck levels.", action: "The Awaiting level's approver chips show mailto icons — click to email that approver directly, and a copy-icon to grab their email address." },
    ],
    hasSlideshow: false,
    hasTour: false,
  },
  {
    id: "summary-sheet-refresh",
    title: "Excel Summary Sheet — Billing Entity, Approval Matrix & Import Behaviour",
    description: "See what's new in the Excel export Summary sheet and which fields are also read back on Smart Import.",
    duration: "3 min",
    category: "Reports",
    updated: "Jul 2026 · Iter 79",
    icon: FileSpreadsheet,
    color: "bg-emerald-600",
    steps: [
      { target: "Export any project to Excel.", action: "The Summary sheet now includes Billing Entity, Bid Category, Forecasted Closure Date, Competencies, Commercial Status and computed Effective Margin %." },
      { target: "Scroll to the 'APPROVAL MATRIX & HISTORY' block.", action: "Lists every configured level with its approvers, plus (after any approvals) an Approval History table with Level, Approver, Approved At, Comments." },
      { target: "Smart Import back.", action: "On Smart Import, the same fields (Billing Entity, Bid Category, Forecasted Closure Date, Competencies, Commercial Status) are extracted from the Summary sheet and matched to your master data by name." },
      { target: "Approver info is export-only.", action: "Approver names, timestamps, and comments are never imported — those come from real actions inside the app, so importing an old file will never overwrite the live approval history." },
    ],
    hasSlideshow: false,
    hasTour: false,
  },
