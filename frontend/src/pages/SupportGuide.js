...
mongosh --eval "db.adminCommand('ping')"`}
          ...
          <Section id="release-notes" title="15. Release Notes (2025)" updated="Jul 2026 · Iter 64">
            ...
            <h3 className="text-lg font-semibold text-[#10B981] mt-6">15.7 Iteration 68–72 — UX &amp; Export Polish</h3>
            <ul className="list-disc pl-6 space-y-1 text-sm">
              <li><strong>Iter 75–76 — Effective Margin formula correction</strong>: <code>calculateWaveSummary</code> and <code>calculateOverallSummary</code> in <code>/utils/estimatorCalcs.js</code> now compute margin as <code>100% − (Net Cost / Resources Price) × 100</code> where <code>Net Cost = totalCostToCompany − logistics.totalLogistics</code> and <code>Resources Price = totalRowsSellingPrice + amsSharedAnnual</code>. Both sides symmetric wrt logistics (excluded) and AMS (included). Fixes two prior asymmetries: (i) logistics inflated revenue but not CTC, (ii) AMS internal cost was in numerator but AMS revenue was missing from denominator.</li>
              <li><strong>Iter 77 — Margin Breakdown tooltip</strong>: New <code>marginDeviations</code> array is populated per wave (T&amp;M rows with <code>override_hourly_rate &gt; 0</code>, AMS buckets where <code>hourly_rate ≠ cost_rate / (1 − margin)</code>) and aggregated into <code>overall.marginDeviations</code>. <code>OverallSummary.js</code> wraps the effective-margin figure in a Shadcn Tooltip that lists each deviation with a coloured left border (green = boost, red = drag), wave name, expected vs. actual price, and signed $ delta.</li>
            </ul>
            <ul className="list-disc pl-6 space-y-1 text-sm">
              <li><strong>Iter 68 — Excel Summary duplicate AMS fix</strong>: <code>excelExport.js::buildSummarySheet</code> was rendering a second "AMS SHARED SUPPORT ROLL-UP" block right after the Grand Total row. The redundant loop was removed; AMS totals still flow into the Grand Total exactly once. Verified via the testing agent + a manual UI download.</li>
              <li><strong>Iter 69 — Wave Grid split-pane</strong>: The single-table Wave Grid in <code>WaveContent.js</code> was replaced with a flex container hosting two independently-scrolling tables. Left pane is a fixed 622px width (drag-handle, #, Skill, Level, Location, $/Month, Onsite, Travel, Grp) with vertical scroll only. Right pane is <code>flex-1</code> with both axes scrollable. Vertical scroll is mirrored via <code>onScroll</code> refs; row + header heights are aligned via a <code>useLayoutEffect</code> pass. Row hover uses a shared <code>hoveredRowId</code> state so the highlight lights up both panes. <code>DragDropContext</code> still wraps the whole component; <code>Droppable</code> + <code>Draggable</code> live on the left pane so resource-row reordering is unaffected. A new CSS utility <code>.no-scrollbar</code> is defined in <code>App.css</code> to hide the left pane scrollbar.</li>
              <li><strong>Iter 70 — Excel filename enrichment</strong>: <code>buildExportWorkbook</code> now returns <code>&#123;PRJ-NNNN&#125;_&#123;Customer&#125;_&#123;ProjectName&#125;_v&#123;N&#125;_Estimate.xlsx</code>. A sanitizer strips invalid FS chars (<code>\/:*?"&lt;&gt;|</code> + control chars), collapses whitespace to <code>_</code>, dedupes consecutive underscores, and trims edges. Customer capped at 30 chars, project name at 50.</li>
              <li><strong>Iter 71 — Consistent filename naming across exports</strong>: All four export flows now share <code>buildExportFilename()</code> in <code>/utils/filename.js</code>:
                <ul className="list-circle pl-6 mt-1 text-xs">
                  <li><code>excelExport.js::buildExportWorkbook</code> → <code>..._Estimate.xlsx</code></li>
                  <li><code>CashflowStatement.js::exportCashflowExcel</code> → <code>..._Cashflow.xlsx</code></li>
                  <li><code>PaymentMilestones.js::exportMilestonesExcel</code> → <code>..._Milestones.xlsx</code></li>
                  <li><code>GanttCard.js::exportGanttPNG / exportGanttExcel</code> → <code>..._Gantt.png</code> / <code>..._Gantt.xlsx</code></li>
                </ul>
                Regression fix: an earlier version used the free-form Project Description in the middle slot (which contained noisy values like <code>"22"</code> on some legacy projects); the slot now uses the Project <em>Name</em>. The Gantt PNG had a leftover hardcoded <code>"gantt-chart.png"</code> that was replaced with the builder. Backend <code>/api/projects/&#123;id&#125;/cashflow</code> was extended to include <code>customer_name</code>, <code>description</code>, and <code>version</code> so the frontend can generate the filename without a second fetch. <code>ProjectEstimator.js</code> now passes <code>projectNumber</code>, <code>projectName</code>, <code>customerName</code>, <code>projectDescription</code>, <code>projectVersion</code> as props to <code>GanttCard</code>.</li>
              <li><strong>Iter 72 — Draggable Wave Tabs</strong>: The <code>TabsList</code> in <code>ProjectEstimator.js</code> is wrapped in <code>@hello-pangea/dnd</code>'s <code>DragDropContext</code> + horizontal <code>Droppable</code>. Each <code>TabsTrigger</code> is a <code>Draggable</code> whose entire body is the drag handle. A small <code>GripVertical</code> icon appears when <code>waves.length &gt; 1 &amp;&amp; !isReadOnly</code>. On drop, <code>handleWaveTabDragEnd</code> splices the waves array and toasts the new position. Because <code>buildGanttRows(waves)</code>, Excel Summary/Wave sheets, Overall Summary, Cashflow and Milestones all iterate the waves array in-order, no additional plumbing is required to keep them in sync. Tab click semantics are preserved (drag threshold gating in the library).</li>
              <li><strong>Test coverage</strong>: filename regression + wave reorder verified via the testing agent iterations 68–72 and manual UI checks (see <code>/app/test_reports/iteration_68.json</code> onwards).</li>
            </ul>

            <h3 className="text-lg font-semibold text-[#10B981] mt-6">15.8 Iter 78–79 — Billing Entity, Approval Matrix &amp; In-App Approvals</h3>
            <ul className="list-disc pl-6 space-y-1 text-sm">
              <li><strong>Billing Entities master</strong>: new collection <code>billing_entities</code>, CRUD routes at <code>/api/billing-entities</code>, admin-only mutations. Every project now stores <code>billing_entity_id</code> and <code>billing_entity_name</code>. Backfill script assigned "Yash Technologies Middle East FZ LLC" to legacy rows.</li>
              <li><strong>Multi-level sequential Approval Matrix</strong>: <code>approval_matrices</code> collection stores levels per billing entity. On <code>POST /api/projects/&#123;id&#125;/submit-for-review</code> (Iter 82 code path in <code>projects.py</code>), the endpoint reads the matrix, dedupes approvers by email, groups by level, and persists <code>matrix_levels</code>, <code>current_approval_level=1</code>, and empty <code>approval_history</code>. <em>Only Level 1</em> approvers receive an email + in-app notification at submit time.</li>
              <li><strong>Level advancement</strong>: <code>POST /api/projects/&#123;id&#125;/approve</code> appends to <code>approval_history</code>, looks up the next higher entry in <code>matrix_levels</code>, and either (a) advances <code>current_approval_level</code> + emails that level's approvers, or (b) closes the workflow (<code>status = "approved"</code>). Rejection short-circuits the whole ladder.</li>
              <li><strong>Model exposure fix</strong>: added <code>matrix_levels</code>, <code>matrix_approvers</code>, <code>current_approval_level</code>, <code>approval_history</code> to Pydantic <code>Project</code> in <code>models.py</code> so GET routes return them (previously stripped by <code>extra="ignore"</code>).</li>
              <li><strong>Sidebar reorg</strong>: <code>Layout.js</code> — Approval Matrix menu item moved from <code>masterDataItems</code> to <code>adminItems</code>, visible only when <code>user.role === "admin"</code>.</li>
              <li><strong>Approval Progress tracker</strong>: new component <code>components/estimator/ApprovalStatusTracker.js</code>, rendered above <code>ProjectInfoCard</code> in <code>ProjectEstimator.js</code>. Collapsible header. Level cards show status (Approved/Awaiting/Pending/Rejected) with approver chips (copy-email + mailto icons on the awaiting level) and, for approved levels, the actual approver + timestamp + comment from <code>approval_history</code>.</li>
              <li><strong>Submit dialog UX</strong>: <code>SubmitReviewDialog</code> in <code>EstimatorDialogs.js</code> now accepts a <code>matrixLevels</code> prop. When present, the "Select Approver" dropdown is hidden and replaced by a preview of all levels + approvers. Backend <code>submit-for-review</code> signature was relaxed to <code>approver_email: str = ""</code>.</li>
              <li><strong>Notification bell inline actions</strong>: <code>Layout.js</code> — for any <code>review_request</code> notification with a <code>project_id</code>, inline Approve/Reject/Open buttons render below the message. Approve/Reject expand a comment textarea in-place and POST to <code>/api/projects/&#123;id&#125;/approve</code> or <code>/reject</code>, then mark the notification as read and refresh.</li>
              <li><strong>Excel export additions</strong>: Summary sheet in <code>excelExport.js</code> now emits Billing Entity, Bid Category, Forecasted Closure Date, Competencies, Commercial Status and Effective Margin. An "Approval Matrix &amp; History" block lists every configured level + approvers, followed by an Approval History table (Level, Approver, Approved At, Comments).</li>
              <li><strong>Smart Import extension</strong>: <code>excelImport.js</code> label-scans the Summary sheet and returns <code>billingEntityName</code>, <code>bidCategory</code>, <code>forecastedClosureDate</code>, <code>competencyNames</code>, <code>commercialStatus</code>. <code>ProjectEstimator.js::confirmSmartImport</code> matches these to master data by name and applies them (both replace-current and import-as-new-version paths). Approver names/timestamps/comments are export-only and never re-imported.</li>
            </ul>
