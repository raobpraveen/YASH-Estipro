...
106. **Background emails on /new-version**: Refactored `create_new_version` to take a `BackgroundTasks` param. Notification DB inserts stay inline (so the bell shows them on next poll), but every `send_email(...)` was moved to `background_tasks.add_task(...)`. Measured response time on PRJ-0043 new-version: **208 ms** (was 15-60 s under SMTP timeouts).
107. **Approval Escalation Timer**: New module `backend/approval_escalation.py` — asyncio loop kicked off in a `startup` handler that scans every `APPROVAL_ESCALATION_INTERVAL_HOURS` hours (default 6) for projects with `status="in_review"` whose current level has been waiting longer than `APPROVAL_ESCALATION_DAYS` (default 2). Fires a reminder email + `review_request` notification to the current level's approvers and stamps `project.escalation_reminders["level_N"]` so the same reminder doesn't repeat within `APPROVAL_ESCALATION_COOLDOWN_HOURS` (default 24). Manual trigger endpoint `POST /api/admin/run-escalation-scan` for on-demand runs / testing.
108. Verified end-to-end by testing agent (iteration_70.json) — 100% backend + 100% frontend pass, no issues.

### Iteration 83–84: Projects list + approval flow bug bash (Completed Jul 2026)
98. **Projects list — group-aware resource count**: `calculateProjectValue.resourceCount` in `Projects.js` now dedupes allocations that share the same non-empty `resource_group_id`; PRJ-0042 / PRJ-0043 correctly show Resources = 4 (was 6).
99. **Projects list — Totals row alignment**: Totals label now spans columns 1-6 (Project# → Status); Resources (integer sum), Man-Months (2dp) and Grand Total ($ formatted) sit exactly under their headers; last two columns (Created, Actions) are blank.
100. **Backend approve endpoint — matrix-aware authorization + idempotency**: `POST /api/projects/{id}/approve` now (a) rejects callers not present in the CURRENT level's approver list with 403 and (b) rejects a repeat approve from the same caller at the same level with 409. Fixes the reported "L1 approver approving twice reflects as final approval" bug.
101. **Frontend `isDesignatedApprover` — matrix-aware**: `ProjectEstimator.js` now derives the flag from `matrixLevels[current_approval_level].approver_emails` and honours idempotency via `approval_history`. After L1 approves, the Approve button hides for the L1 user and shows for the L2 approver.
102. **New-version routing (Option B)**: `create_new_version` now inspects the caller's level in the parent's matrix. **L1 approver edits** → new version auto-submits at Level 2 with a synthetic `approval_history` entry `"L1 already approved on previous version v{n}. Carried over automatically after edits by the L1 approver."`; L2 approvers receive email + `review_request` notification. **L2+ approver edits** → new version stays as draft; L1 approvers (and project creator when known) receive an email alert + `version_alert` notification instructing them to review changes and manually resubmit — flow restarts from L1 on submit.
103. **Tested end-to-end**: iteration_69.json — 10/10 backend tests passing, all frontend UI checks passing. Minor perf note: email fan-out on new-version can be moved to a background task (not a functional bug).

### Iteration 82: Group-aware Resource Count + Clone approval reset (Completed Jul 2026)
95. **Wave grid resource count fix**: `WaveContent.js` header now uses `waveSummary.uniqueResourceCount` (which groups rows sharing the same `resource_group_id` as one physical resource) instead of the raw `grid_allocations.length`. Fixes the case where a resource split across onsite/offshore rows was double-counted.
96. **Group-aware Onsite / Traveling counts**: `calculateWaveLogistics` in `estimatorCalcs.js` now dedupes onsite and traveling counts by `resource_group_id`, so flight/visa costs (which multiply by these counts) no longer double-charge grouped rows. `offshoreResourceCount` derived as `uniqueResourceCount − onsiteResourceCount`.
97. **Clone endpoint reset**: `/api/projects/{id}/clone` now also resets `matrix_levels`, `matrix_approvers`, `current_approval_level`, `approval_history`, `previous_status`, and `commercial_status` so a clone starts from a truly clean approval slate. Existing incorrectly-cloned projects (name contains `(Copy)` + `parent_project_id` set + `approval_history` populated) were backfilled once via a one-off script.

### Iteration 81: Reject textarea fix in notification bell (Completed Jul 2026)
94. **Bug fix (Iter 80 follow-up)**: In `Layout.js`, the `isExpanded` guard on notification items only matched `expandedActionId === notif.id`, but the Reject flow sets `expandedActionId = "reject-${notif.id}"`. Changed to `isExpanded = expandedActionId === notif.id || expandedActionId === "reject-" + notif.id` so the comment textarea + Confirm Reject button now render as expected. Verified by re-screenshot after the testing agent flagged this in iteration_68.json.

### Iteration 80: Excel Export/Import completeness + In-App Approvals + Help Docs (Completed Jul 2026)
91. **Excel export additions**: Summary sheet now shows Billing Entity, Bid Category, Forecasted Closure Date, Competencies, Commercial Status and computed Effective Margin %. Approval Matrix &amp; History block (levels + approvers, plus an Approval History table with Approver / Approved At / Comments) is included when the project has a matrix and history.
92. **Smart Import extension**: `excelImport.js` label-scans the Summary sheet and returns `billingEntityName`, `bidCategory`, `forecastedClosureDate`, `competencyNames`, `commercialStatus`. `ProjectEstimator.js` matches them to master data by name and applies them on both replace-current and import-as-new-version paths. Approver names/dates/comments are export-only and never re-imported.
93. **In-App Approve/Reject from Notification Bell**: `Layout.js` — every `review_request` notification now shows inline Approve / Reject / Open buttons. Approve opens a compact comment textarea; Reject shows a required "reason" textarea; both post to `/api/projects/{id}/approve` or `/reject`, mark the notification as read, and refresh. Existing project-page approver flow is unchanged.
### Iteration 80: Excel Export/Import completeness + In-App Approvals + Help Docs (Completed Jul 2026)
91. Excel export/import completeness + in-app approvals + help docs (see Iter 80 above — kept in place, this was reordered when Iter 81/82 fixes were inserted).

### Iteration 79: Sequential Submit UX + Approver-wise Approval Progress Tracker (Completed Jul 2026)
87. **Submit-for-Review dialog UX**: When the project's Billing Entity has an Approval Matrix configured, the "Select Approver" dropdown is now **hidden** and replaced with a compact preview showing every level, its label, and its approvers — with a "Notified first" tag on Level 1. If no matrix exists the classic dropdown still shows. Backend `submit_for_review` accepts empty `approver_email` and falls back to Level 1 approvers when a matrix exists.
88. **New component**: `ApprovalStatusTracker.js` renders **above Project Information** when the project is in_review/approved/rejected AND has a matrix. Header is **collapsible** (click to toggle). Shows level-by-level status (Approved / Awaiting / Pending / Rejected), each approver as a chip with **Copy email** and **Follow-up mailto** icons for the current awaiting level, plus timestamp + comments of who actually approved each level.
89. **Backend model**: Added `matrix_levels`, `matrix_approvers`, `current_approval_level`, `approval_history` fields to Pydantic `Project` model so the GET endpoint returns them (previously stripped by `extra="ignore"`).
90. **Estimator page**: Loads full matrix on project load (to enrich `approver_names` + `label` for the tracker), refreshes after every submit/approve/reject so the tracker updates when a level advances.
